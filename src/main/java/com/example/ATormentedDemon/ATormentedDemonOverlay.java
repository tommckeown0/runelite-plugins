...
public class ATormentedDemonOverlay extends Overlay {
    ...
    private final ATormentedDemonPlugin plugin;

    @Inject
    ATormentedDemonOverlay(Client client, ATormentedDemonPlugin plugin) {
        this.client = client;
        this.plugin = plugin;
    ...
    public Dimension render(Graphics2D graphics) {
        if (!plugin.hasActiveFireBombWarning()) {
            return null;
        }

        for (WorldPoint safe : plugin.getSafeTiles()) {
            outlineTile(graphics, safe, SAFE_BORDER, SAFE_FILL);
        }
        for (WorldPoint danger : plugin.getFireBombDangerTiles()) {
