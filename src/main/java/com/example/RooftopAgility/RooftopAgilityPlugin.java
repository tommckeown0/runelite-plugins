...
import com.example.PiggyUtils.API.InventoryUtil;
import com.example.PiggyUtils.BreakHandler.ReflectBreakHandler;
...
@PluginDescriptor(
        name = "A Rooftop Agility",
        description = "Rooftop agility helper plugin",
...
public class RooftopAgilityPlugin extends Plugin {
    ...
    private void findObstacle() {
        ...
        Obstacles obstacle = Obstacles.getObstacle(playerLoc);
        log.info("[RooftopAgility] findObstacle - Player at: " + playerLoc);

        if (obstacle == null) {
            log.info("[RooftopAgility] No obstacle found for current location");
            return;
        }

        log.info("[RooftopAgility] Matched obstacle: " + obstacle.name() + " (ID: " + obstacle.getObstacleId() + ")");

        Optional<TileObject> tileObject = TileObjects.search().withId(obstacle.getObstacleId()).nearestToPlayer();
        log.info("[RooftopAgility] TileObject found: " + tileObject.isPresent() + ", Player interacting: " + client.getLocalPlayer().isInteracting());

        if (tileObject.isPresent()) {
            TileObject obj = tileObject.get();
            log.info("[RooftopAgility] Obstacle location: " + obj.getWorldLocation() + ", ID: " + obj.getId() + ", Type: " + obj.getClass().getSimpleName());
            String[] actions = TileObjectQuery.getObjectComposition(obj).getActions();
            log.info("[RooftopAgility] Available actions: " + java.util.Arrays.toString(actions));

            if (!client.getLocalPlayer().isInteracting()) {
                String action = actions[0];
                log.info("[RooftopAgility] Clicking obstacle with action: " + action);
                log.info("[RooftopAgility] Object class: " + obj.getClass().getName());
                log.info("[RooftopAgility] World coords: " + obj.getWorldLocation());
                log.info("[RooftopAgility] getX/getY (scene): " + obj.getX() + ", " + obj.getY());

                // Try the standard interaction
                MousePackets.queueClickPacket();
                ObjectPackets.queueObjectAction(1, obj.getId(), obj.getWorldLocation().getX(), obj.getWorldLocation().getY(), false);
            }
        }
    }
